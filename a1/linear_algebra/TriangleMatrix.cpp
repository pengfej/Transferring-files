...
TriangleMatrix::TriangleMatrix(std::size_t nrows, std::size_t ncols, const double val)
{
...
DenseVector TriangleMatrix::operator*(const DenseVector& vec) const
{
  assert(m_is_allocated);
  if (m_cols != vec.size())
    throw std::runtime_error("Error in \"TriangleMatrix::operator*\" "
...
DenseVector TriangleMatrix::trans_mult(const DenseVector& vec) const
{
  assert(m_is_allocated);
  if (m_cols != vec.size())
    throw std::runtime_error("Error in \"TriangleMatrix::operator*\" "
